...
func testGateway(t *testing.T, syncDir string, ignorePatterns []string, user, pass string) string {
	t.Helper()
	metaDir := t.TempDir()
...
func sftpClient(t *testing.T, addr, user, pass string) ssh.Channel {
	...
	// Use a combined channel-like struct.
	ch := &sshPipe{w: stdin, r: stdout}
	return ch
}

// sshPipe adapts session stdin/stdout to look like an ssh.Channel for our test helpers.
type sshPipe struct {
	w io.WriteCloser
	r io.Reader
}
...
func TestFstat(t *testing.T) {
	...
	_, rest, _ := unmarshalUint32(payload)
	// Parse attrs: flags(4) + size(8).
	_, rest, _ = unmarshalUint32(rest)
...
func TestStatIgnoredPath(t *testing.T) {
	syncDir := t.TempDir()
	os.WriteFile(filepath.Join(syncDir, "secret.log"), []byte("x"), 0o644)

	addr := testGateway(t, syncDir, []string{"*.log"}, "user", "pass")
	ch := sftpClient(t, addr, "user", "pass")
	sftpInit(t, ch)

	pktType, payload := sftpStat(t, ch, 1, "/secret.log")
	if pktType != sshFxpStatus {
		t.Fatalf("expected STATUS, got %d", pktType)
	}
	_, rest, _ := unmarshalUint32(payload)
	...
	if code != sshFxPermissionDenied {
		t.Errorf("expected PERMISSION_DENIED for ignored, got %d", code)
	}
}
...
func TestOpenIgnoredPath(t *testing.T) {
	syncDir := t.TempDir()
	os.WriteFile(filepath.Join(syncDir, "hidden.log"), []byte("x"), 0o644)
	...
	var req []byte
	req = marshalUint32(req, 1)
	req = marshalString(req, "/hidden.log")
	req = marshalUint32(req, sshFxfRead)
	req = marshalUint32(req, 0)
	writePacket(ch, sshFxpOpen, req)
...
func TestReaddirSecondCallEOF(t *testing.T) {
	...
	// Second READDIR with same handle should fail (handle was consumed).
	var req []byte
	...
	_, rest, _ := unmarshalUint32(payload)
	code, _, _ := unmarshalUint32(rest)
	if code != sshFxFailure && code != sshFxEOF {
		t.Errorf("expected FAILURE or EOF on consumed handle, got %d", code)
	}
}
...
func TestListDirectoryIgnored(t *testing.T) {
	syncDir := t.TempDir()
	os.WriteFile(filepath.Join(syncDir, "keep.txt"), []byte("keep"), 0o644)
	os.WriteFile(filepath.Join(syncDir, "secret.log"), []byte("secret"), 0o644)
	...
	if len(names) != 1 || names[0] != "keep.txt" {
		t.Errorf("expected [keep.txt], got %v", names)
	}
}
...
func TestPathTraversal(t *testing.T) {
	...
	// Cleaned paths like /../../../etc/passwd resolve to /etc/passwd inside syncDir,
	// which is safe (just returns no-such-file). Verify traversal doesn't escape.
	...
	// Verify the file was NOT actually read from /etc/passwd.
...
func TestNoAuthMode(t *testing.T) {
	syncDir := t.TempDir()
	addr := testGateway(t, syncDir, nil, "", "")
